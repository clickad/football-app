...
function homeCtrl($http, fixtureService, competitionsService, $filter, standingsService, $timeout, $window) {
  ...
  var getFixtures = function getFixtures() {
    fixtureService.getFixtures().then(function (fixtures) {
      vm.games = fixtures.fixtures; // Get competition names

      var getCompetitions = function getCompetitions() {
      ...
      getCompetitions(); // Adjust some data as month and day to show '0' in front if less then '10', same as if resul null show '-'
